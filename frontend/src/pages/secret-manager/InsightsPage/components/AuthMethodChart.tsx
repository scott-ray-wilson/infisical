...
export const AuthMethodChart = () => {
  ...
  return (
    <UnstableCard className="mt-6 w-full">
      <UnstableCardHeader>
      ...
      <UnstableCardContent>
        ...
        {!isPending && methods.length === 0 && (
          <p className="py-8 text-center text-sm text-muted">No secret access data available</p>
        )}
        {!isPending && methods.length > 0 && (
              ...
              {methods.map((entry, index) => {
                ...
                return (
                  <div key={entry.method} className="flex items-center gap-3">
                    ...
                    />
                    <span className="flex-1 text-sm">{entry.method}</span>
