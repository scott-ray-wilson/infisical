...
type TSecretQueueFactoryDep = {
  ...
  secretSyncQueue: Pick<TSecretSyncQueueFactory, "triggerSecretSyncsByPath">;
};
...
export const secretQueueFactory = ({
  ...
  queueService.start(QueueName.SecretSync, async (job) => {
    ...
    await secretSyncQueue.triggerSecretSyncsByPath({ projectId, environmentSlug: environment, secretPath });
