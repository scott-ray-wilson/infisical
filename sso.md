...
| Provider | Bring up | Seed | Provider URL | Seeded users (password `Passw0rd!`) |
| --- | --- | --- | --- | --- |
| Keycloak (OIDC) | `make up-dev-oidc` | seeded on boot, re-seed with `make seed-dev-oidc` | http://localhost:8088 (admin / admin) | jdoe@infisical.com, asmith@infisical.com |
...
every start (run `make seed-dev-oidc` to force a re-import without a full `make down`).
...
| Seeded users | `jdoe@infisical.com`, `asmith@infisical.com` (password `Passw0rd!`) |

### Discovery URL (read the networking note)

The OIDC discovery URL must resolve to the **same issuer** from both your browser and the
Infisical backend, otherwise token validation fails with an issuer mismatch. Which URL you
use depends on where the backend runs:

- **Backend in Docker (the default for `make up-dev-oidc`):** use the `keycloak.local`
  hostname, the same trick the PingFederate container uses. The backend already has
  `keycloak.local` in its `extra_hosts`; you just need your host to resolve it too. Add this
  line to your machine's `/etc/hosts`:
  ...
  ```
  http://keycloak.local:8088/realms/infisical/.well-known/openid-configuration
  ```

- **Backend on the host** (`cd backend && npm run dev`, not in compose): `localhost` resolves
  the same for the browser and the backend, so no `/etc/hosts` edit is needed:
...
| Seeded users | `jdoe`, `asmith` (password `Passw0rd!`) | same |
...
curl -X POST http://localhost:8080/api/v1/sso/oidc/config \
  ...
  -d '{
    ...
    "discoveryURL": "http://keycloak.local:8088/realms/infisical/.well-known/openid-configuration",
...
curl -X POST http://localhost:8080/api/v1/sso/ldap/login \
  ...
  -d '{ "organizationSlug": "infisical", "username": "jdoe", "password": "Passw0rd!" }'
```
...
| OIDC fails with an issuer / `iss` mismatch | Browser and backend resolved Keycloak via different hostnames. Use `keycloak.local:8088` everywhere (add it to `/etc/hosts`) when the backend runs in Docker, or `localhost:8088` everywhere when it runs on the host. |
| Redirected to a broken page after Keycloak login | `SITE_URL` does not match the app URL. Set `SITE_URL=http://localhost:8080`. |
| Backend cannot reach the discovery URL | Inside Docker `localhost:8088` is the backend itself. Use `keycloak.local:8088` (resolved via `extra_hosts`) or `ldap://openldap:389` for LDAP. |
