...
const main = async () => {
  ...
  const bindDN = process.env.SEED_LDAP_BIND_DN || "cn=admin,dc=acme,dc=com";
  const bindPass = process.env.SEED_LDAP_BIND_PASS || "admin";
  const searchBase = process.env.SEED_LDAP_SEARCH_BASE || "ou=people,dc=acme,dc=com";
  const groupSearchBase = process.env.SEED_LDAP_GROUP_SEARCH_BASE || "ou=groups,dc=acme,dc=com";
