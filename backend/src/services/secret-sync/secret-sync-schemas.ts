...
const BaseSyncOptionsSchema = <T extends AnyZodObject | undefined = undefined>({
  ...
  if (isUpdateSchema) {
    void schema.describe(SecretSyncs.UPDATE(destination).syncOptions).optional();
  } else {
    void schema.describe(SecretSyncs.CREATE(destination).syncOptions);
  }

  // Explicitly type the return value
  return schema as T extends AnyZodObject
