  ...
  syncSecrets: async (secretSync: TAwsParameterStoreSyncWithCredentials, secretMap: TSecretMap) => {
    ...
    for await (const entry of Object.entries(secretMap)) {
      ...
      if (
        ...
        destinationConfig.keyId !== awsParameterStoreMetadataRecord[key]?.KeyId
      ) {
        try {
      ...
      logger.warn(tagKeysToRemove, "Tags to remove");
      logger.warn(tagsToAdd, "Tags to add");
