...
import { IdentitiesSchema, IdentityProjectMembershipsSchema, ProjectUserMembershipRolesSchema } from "@app/db/schemas";
import { ApiDocsTags, IDENTITIES, ORGANIZATIONS, PROJECT_IDENTITIES } from "@app/lib/api-docs";
...
export const registerIdentityProjectV2Router = async (server: FastifyZodProvider) => {
  server.route({
    method: "POST",
    ...
    schema: {
      ...
      description: "Create project identity",
      security: [
        {
          bearerAuth: []
        }
      ...
      body: z.object({
        ...
        metadata: z
          ...
          .array()
          .optional()
          .describe(IDENTITIES.CREATE.metadata),
        roles: z
          .array(
            z.union([
              z.object({
      ...
      response: {
        200: z.object({
          projectIdentity: IdentitiesSchema.extend({
    ...
    handler: async (req) => {
      const projectIdentity = await server.services.identityProjectV2.createProjectIdentity({
