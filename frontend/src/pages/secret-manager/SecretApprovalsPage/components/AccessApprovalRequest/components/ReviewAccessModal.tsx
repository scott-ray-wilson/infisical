...
const getReviewedStatusSymbol = (status?: ApprovalStatus) => {
  if (status === ApprovalStatus.APPROVED)
    return <FontAwesomeIcon icon={faCheckCircle} size="xs" className="text-green" />;
  if (status === ApprovalStatus.REJECTED)
    return <FontAwesomeIcon icon={faXmarkCircle} size="xs" className="text-red" />;
    ...
    <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="">
          ...
          <div className="thin-scrollbar max-h-[40vh] overflow-y-auto rounded p-2">
                      ...
                      <div>
                        ...
                        <div>
                          {approver?.user
                            ?.map(
                              (el) =>
                                approverSequence?.membersGroupById?.[el.id]?.[0]?.user?.username
                            )
                            .join(",") || "-"}
                        </div>
                      </div>
