...
import { TCreateProjectIdentityV2DTO } from "@app/services/identity-project/identity-project-v2-types";
...
type TIdentityProjectServiceFactoryDep = {
  ...
  identityOrgMembershipDAL: Pick<TIdentityOrgDALFactory, "create">;
  identityDAL: Pick<TIdentityDALFactory, "create">;
  identityMetadataDAL: Pick<TIdentityMetadataDALFactory, "insertMany">;
  permissionService: Pick<TPermissionServiceFactory, "getProjectPermission" | "getProjectPermissionByRole">;
};

export type TIdentityProjectV2ServiceFactory = ReturnType<typeof identityProjectV2ServiceFactory>;

export const identityProjectV2ServiceFactory = ({
  identityProjectDAL,
  permissionService,
  ...
  const createProjectIdentity = async ({
    ...
    const projectIdentity = await identityProjectDAL.transaction(async (tx) => {
      const identity = await identityDAL.create({ name, hasDeleteProtection, orgId: actorOrgId, projectId }, tx);
